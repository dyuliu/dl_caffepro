...
namespace caffepro {
	softmax_loss_ohem_layer::softmax_loss_ohem_layer(caffepro_context *context, const LayerParameter &param)
	...
	softmax_loss_ohem_layer::~softmax_loss_ohem_layer() {
		release_all();
	}

	void softmax_loss_ohem_layer::init() {
		check_input();
		output_top_n_ = 0;
		...
		ohem_size_ = layer_param_.softmaxlossohem_param().ohem_size();
		force_random_ = layer_param_.softmaxlossohem_param().force_random();
	}

	void softmax_loss_ohem_layer::resize() {
		check_input();
	...
	void softmax_loss_ohem_layer::on_forward(int device_index) {
		...
		if (ohem_size_ < num) {
			if (force_random_) {
				...
				for (int i = 0; i < ohem_size_; i++)
					bp_indicator_data[idx[i]] = 0.f; // largest confidence; do not back prop
				...
				for (int i = 0; i < num; i++, avg_prob_data += feature_dim) {
					...
					score_max[i] = std::make_pair(s, i);

					//printf("i: %d, s: %.10f --- i: %d, s: %.10f\n", i, s, score_max[i].second, score_max[i].first);
				}
				...
				data_type *bp_indicator_data = bp_indicator_->get(device_index)->mutable_cpu_data();
				for (int i = 0; i < ohem_size_; i++)
					bp_indicator_data[score_max[i].second] = 0.f; // largest confidence; do not back prop

				for (int i = ohem_size_; i < num; i++)
					bp_indicator_data[score_max[i].second] = 1.f; // back prop
	...
	void softmax_loss_ohem_layer::on_backward(int device_index, act_selector bp_acts, weight_selector bp_weights, act_selector clear_acts_diff, weight_selector clear_weights_diff) {

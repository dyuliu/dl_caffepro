...
namespace caffepro {
	eltwise_amax_layer::eltwise_amax_layer(caffepro_context *context, const LayerParameter &param)
	...
	eltwise_amax_layer::~eltwise_amax_layer() {
		release_all();
	}

	void eltwise_amax_layer::init() {
		check_input();
	}

	__global__ static void eltamax_forward(int n, const data_type *input1, const data_type *input2, data_type *output) {
		CUDA_KERNEL_LOOP(index, n) {
			if (fabsf(input1[index]) >= fabsf(input2[index])) {
	...
	void eltwise_amax_layer::on_forward(int device_index) {
		auto &input1 = *inputs_[0]->get(device_index);
		auto &input2 = *inputs_[1]->get(device_index);
		auto &output = *outputs_[0]->get(device_index);
		KERNEL_CALL(eltamax_forward, input1.count())(input1.count(), input1.gpu_data(), input2.gpu_data(), output.mutable_gpu_data());
		CUDA_POST_KERNEL_CHECK;
	}

	__global__ static void eltamax_backward(int n, const data_type *input1_data, const data_type *input2_data, const data_type *diff,
		...
		CUDA_KERNEL_LOOP(index, n) {
			if (fabsf(input1_data[index]) >= fabsf(input2_data[index])) {
				if (bp1) {
					if (scale_target1 == 0) {
	...
	void eltwise_amax_layer::on_backward(int device_index, act_selector bp_acts, weight_selector bp_weights, act_selector clear_acts_diff, weight_selector clear_weights_diff) {
		if (should_bp(bp_acts, 0) || should_bp(bp_acts, 1)) {
			...
			KERNEL_CALL(eltamax_backward, input1.count())(input1.count(), input1.gpu_data(), input2.gpu_data(), output.gpu_diff(), input1.mutable_gpu_diff(), input2.mutable_gpu_diff(),
				beta1, beta2, bp1, bp2);

...
namespace caffepro {
	class pooling_layer : public caffepro_layer {
	public:
		pooling_layer(caffepro_context *context, const LayerParameter &param);
		~pooling_layer();
	...
	protected:
		// cudnn handles
		struct cudnn_pool_context {
			cudnnPoolingDescriptor_t pooling_desc;
			cudnnTensorDescriptor_t bottom_desc, top_desc;
		};

		std::vector<cudnn_pool_context> pool_contexts_;
	};
}
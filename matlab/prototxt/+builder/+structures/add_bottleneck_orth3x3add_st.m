function [result, output_name] = add_bottleneck_orth3x3add_st(prefix, bottom, bottleneck_outputs, outputs, order_3x3, n_add, stride, has_proj, bn_stat)
    assert(nargin >= 5);
    ...
    r3x3weights = '';
    weight3x3_names = cell(1, n_add);
    for i = 1 : n_add
        weight_name3x3 = [prefix, '_3x3weight_', num2str(i)];
        [r, w] = builder.structures.add_householder_higherorder_st(weight_name3x3, path2a_output, [3, 3, bottleneck_outputs, bottleneck_outputs], order_3x3, opt);
        r3x3weights = cat(2, r3x3weights, r);
        weight3x3_names{i} = w;
    end
    weight_name3x3sum = [weight_name3x3, '_sum'];
    rw_3x3sum = builder.layers.add_eltsum(weight_name3x3sum, weight3x3_names, weight_name3x3sum);

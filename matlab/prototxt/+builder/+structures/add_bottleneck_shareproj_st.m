function [result, output_name] = add_bottleneck_shareproj_st(prefix, bottom, bottleneck_outputs, outputs, stride, has_proj, bn_stat)
    assert(nargin >= 4);
    ...
    weight_name = [prefix, '_1x1weight'];
    rw = builder.layers.add_weight(weight_name, bottom, weight_name, [1, 1, outputs, bottleneck_outputs], opt);
    [r2a, path2a_output] = builder.structures.add_correlation_st([prefix, '_branch2a'], bottom, weight_name, stride, 0, false, true, true, bn_stat, opt);
    ...
    trans_weight_name = [weight_name, '_transpose'];
    rwt = builder.layers.add_transpose(trans_weight_name, weight_name, trans_weight_name, 3, [1, 1, bottleneck_outputs, outputs]);
    ...
    result = cat(2, result, rw, r2a, r2b, rwt, r2c);

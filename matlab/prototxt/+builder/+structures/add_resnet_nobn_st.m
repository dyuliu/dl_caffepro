function [result, output_name] = add_resnet_nobn_st(prefix, bottom, outputs, stride, has_proj, bn_stat)
    assert(nargin >= 3);
    ...
    if has_proj
        ...
        [r1, path1_output] = builder.structures.add_conv_st([prefix, '_branch1'], bottom, 1, outputs, stride, 0, true, false, false, bn_stat, opt);
        result = cat(2, result, r1);
    end
    
    result = cat(2, result, '#------1x1-3x3-1x1(', num2str(outputs), ', ', num2str(outputs), ')', nl, nl);
    [r2a, path2a_output] = builder.structures.add_conv_st([prefix, '_branch2a'], bottom, 3, outputs, stride, 1, true, false, true, bn_stat, opt);
    [r2b, path2b_output] = builder.structures.add_conv_st([prefix, '_branch2b'], path2a_output, 3, outputs, 1, 1, true, false, false, bn_stat, opt);

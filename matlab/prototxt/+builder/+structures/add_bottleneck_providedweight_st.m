function [result, output_name] = add_bottleneck_providedweight_st(prefix, bottom, weight_bottom, bottleneck_outputs, outputs, stride, has_proj, bn_stat)
    assert(nargin >= 5);
    ...
    if has_proj
        assert(length(weight_bottom) == 4);
        result = cat(2, result, '#------1x1(', num2str(outputs), ')', nl, nl);
        [r1, path1_output] = builder.structures.add_correlation_st([prefix, '_branch1'], bottom, weight_bottom{4}, stride, 0, false, true, false, bn_stat, opt);
    ...
    [r2c, path2c_output] = builder.structures.add_correlation_st([prefix, '_branch2c'], path2b_output, weight_bottom{3}, 1, 0, false, true, false, bn_stat, opt);

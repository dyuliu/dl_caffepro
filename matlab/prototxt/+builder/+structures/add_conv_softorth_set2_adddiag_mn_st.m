function [result, output_name] = add_conv_softorth_set2_adddiag_mn_st(prefix, bottom, ksize, inputs, outputs, stride, pad, has_bias, has_mn, has_relu, bn_stat, varargin)
    assert(nargin >= 10);
    ...
    if isfield(opt, 'bn_rename_fun'), mn_name = opt.bn_rename_fun(prefix); end;
    ...
    result = cat(2, result, builder.layers.add_diag4d_operation([prefix, '_diag4d'], weight_src_name, weight_src_name, 1, 1));
    ...
    if has_mn 
        result = cat(2, result, builder.layers.add_mn(mn_name, output_name, conv_name, bn_stat), nl);
        output_name = conv_name;
    end

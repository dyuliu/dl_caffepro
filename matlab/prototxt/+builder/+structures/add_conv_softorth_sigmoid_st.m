function [result, output_name] = add_conv_softorth_sigmoid_st(prefix, bottom, ksize, inputs, outputs, stride, pad, has_bias, has_bn, has_relu, bn_stat, varargin)
    assert(nargin >= 10);
    ...
    if has_relu
        result = cat(2, result, builder.layers.add_layer(relu_name, 'sym_sigmoid', output_name, conv_name), nl);
        output_name = conv_name;
    end
end

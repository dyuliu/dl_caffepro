function [result, output_name] = add_bottleneck_householder_st(prefix, bottom, bottleneck_outputs, outputs, stride, has_proj, bn_stat)
    assert(nargin >= 4);
    ...
    [r2bw, householder_weight_name] = builder.structures.add_householder_st(['householder_', prefix, '_branch2b'], path2a_output, [3, 3, bottleneck_outputs, bottleneck_outputs]);
    [r2b, path2b_output] = builder.structures.add_correlation_st([prefix, '_branch2b'], path2a_output, householder_weight_name, 1, 1, false, true, true, bn_stat, opt);
    ...
    result = cat(2, result, r2a, r2bw, r2b, r2c);

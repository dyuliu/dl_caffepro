function [result, output_name] = add_bottleneck_ldropout_st(prefix, bottom, bottleneck_outputs, outputs, init_value, stride, has_proj, bn_stat)
    assert(nargin >= 5);
    ...
    dropout_name = ['dropout', prefix];
    result = cat(2, result, builder.layers.add_learnable_dropout(dropout_name, path2c_output, dropout_name, init_value));
    
    result = cat(2, result, builder.layers.add_eltsum(['interstellar', prefix], {path1_output, dropout_name}, output_name), nl);

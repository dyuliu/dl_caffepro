function [result, output_name] = add_bottleneck_stepgate_st(prefix, bottom, bottleneck_outputs, outputs, gate_init, gate_step, gate_max, gate_start_iter, stride, has_proj, bn_stat)
    assert(nargin >= 7);
    if nargin <= 7, gate_start_iter = []; end;
    if nargin <= 8, stride = 1; end;
    if nargin <= 9, has_proj = false; end;
    ...
    gate_name = ['gate', prefix];
    result = cat(2, result, builder.layers.add_step_gate(gate_name, path2c_output, gate_name, gate_init, gate_step, gate_max, [], [], gate_start_iter));
